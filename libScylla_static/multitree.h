...
public:
    ...
    {
        return CTreeItem(hItem, const_cast<CTreeViewCtrlEx*>(reinterpret_cast<const CTreeViewCtrlEx*>(this)));
    }
    ...
    DWORD SetExtendedTreeStyle(DWORD dwStyle)
    {
        ...
        const DWORD dwOldStyle = m_dwTreeStyle;
    ...
    BOOL IsItemSelected(HTREEITEM hItem)
    {
        ...
        const int iIndex = m_aData.FindKey(hItem);
        if (iIndex >= 0)
        {
    ...
    void _SelectItem(int iIndex, bool bSelect, int action = TVC_UNKNOWN)
    {
        if (iIndex < 0)
            return;
        const bool bSelected = m_aData.GetValueAt(iIndex);
        ...
        if (parent.SendMessage(WM_NOTIFY, nmtv.hdr.idFrom, reinterpret_cast<LPARAM>(&nmtv)) != 0)
            return;
        // Change state
        ...
        parent.SendMessage(WM_NOTIFY, nmtv.hdr.idFrom, reinterpret_cast<LPARAM>(&nmtv));
    }
    ...
    */
    void _SelectBox(CRect rc)
    {
        ...
        while (hItem != nullptr)
        {
            const int i = m_aData.FindKey(hItem);
    ...
    void _DrawDragRect(CPoint pt)
    {
        ...
        dc.DrawDragRect(&rect, szFrame, nullptr/*&rectOld*/, szFrame);
    }
    ...
    void OnChar(UINT nChar, UINT nRepCnt, UINT nFlags)
    {
        if (nChar == VK_SPACE)
        {
            const HTREEITEM hItem = GetFocusItem();
    ...
    void OnLButtonDown(UINT nFlags, CPoint point)
    {
        ...
        if (hItem == nullptr || (hti.flags & TVHT_ONITEMRIGHT) != 0)
        {
    ...
    {
        HTREEITEM hItem = reinterpret_cast<HTREEITEM>(DefWindowProc(uMsg, wParam, lParam));
        if (hItem == nullptr)
            return reinterpret_cast<LRESULT>(hItem);
    ...
    {
        const NMTREEVIEW* lpNMTV = reinterpret_cast<NMTREEVIEW*>(pnmh);
    ...
    {
        NMTVCUSTOMDRAW* lpTVCD = reinterpret_cast<NMTVCUSTOMDRAW*>(lpNMCustomDraw);
        const HTREEITEM hItem = reinterpret_cast<HTREEITEM>(lpTVCD->nmcd.dwItemSpec);
        const int iIndex = m_aData.FindKey(hItem);
        if (iIndex >= 0)
        {
            const bool bSelected = m_aData.GetValueAt(iIndex);
